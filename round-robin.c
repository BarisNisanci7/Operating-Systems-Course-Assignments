    ...
    while (finished_processes < num_processes) {
        ...
        if (!is_empty(queue)) {
            ...
            if (current_process.remaining_burst_time <= quantum) {
                run_process(&cur_time, current_process.remaining_burst_time, &current_process);
                current_process.completion_time = cur_time;
                processes[current_process.id] = current_process;
                finished_processes++;

            } else {
                run_process(&cur_time, quantum, &current_process);
